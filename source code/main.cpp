...
bool keyDown[] = {false,false,false ,false };//设置wsad初始状态，keydown=true表示被按下
...
{
	float dx = x - lastX, dy = y - lastY;//dx,dy鼠标的移动量
	lastX = x, lastY = y;//记录下新鼠标的位置

	if (cameraMoving)//按住右键移动鼠标，移动摄像机，cameraMoving的状态由MousePress决定
	{
		float ratio = 0.1;
        ...
        //rotate:使用从3个标量的轴(此处为y轴）创建的旋转4 * 4矩阵和以度表示的角度(-dx+ratio)来转换mat4()
	...
	else if (selectedIndex>=0)//选中了有效区域
	{
		if (!objectRotating)//不进行旋转操作，按下'r'键—objectRotating=true
		{
			if (selectedIndex >= 0 && selectedIndex < 3)//选中了物体，变换物体位置—objectData.position
			{
		...
		else//objectRotating=true，进行旋转操作，只能对物体，不能对光源
            //改变沿x,y轴方向旋转量rotateX，rotateY，与鼠标移动的距离dx,dy有关
		{
...
void mousePress(int button, int state, int x, int y)
{
	...
	switch (button)
	{
	case GLUT_LEFT_BUTTON://如果鼠标左键被按下
	{
		if (state==GLUT_DOWN)//如果鼠标左键被按住
		{
			selectedIndex = -1;
			vec2 windowSize = { glutGet(GLUT_WINDOW_WIDTH), glutGet(GLUT_WINDOW_HEIGHT) };
			float minDis = 1000;
            //判断是否选中了物体，关键参数：ObjectData.position
			for (int i = 0; i < 3; ++i)
			{
			...
			printf("selected:%d\n", selectedIndex);
            //selectedIndex:3 选中光源
            //selectedIndex:2 选中圆柱体
            //selectedIndex:1 选中球体
            //selectedIndex:0 选中立方体
            //selectedIndex:-1 选中其他区域
		}
	}
	break;
	case GLUT_RIGHT_BUTTON:
	{
		selectedIndex = -1;//不管右键选中什么地方，均不对物体进行操作
		cameraMoving = state == GLUT_DOWN;
        //如果点击鼠标右键并按住移动，即可旋转视角
	}
	break;
	default:
		break;
